...
-- Index for user metadata lookups (phone number is commonly used as login key)
CREATE INDEX IF NOT EXISTS idx_profiles_user_phoneno ON profiles(user_phoneno);
...
CREATE INDEX IF NOT EXISTS idx_profiles_composite ON profiles(id, role);
...
6. Remember to refresh the materialized view periodically if it gets stale:
   REFRESH MATERIALIZED VIEW profile_cache;
*/