...
export function HeroSection() {
  ...
  return (
    <section className="relative bg-gradient-to-br from-brown-50 via-white to-primary/5 overflow-hidden">
      {/* Background Pattern */}
      ...
      <div className="container mx-auto px-4 py-8 sm:py-12 lg:py-20 relative z-10">
        <div className="grid lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 items-center">
          ...
          <div className="space-y-4 sm:space-y-6 text-center lg:text-left order-2 lg:order-1">
            {/* Badge */}
            <div className="inline-flex items-center gap-2 bg-primary/10 text-primary px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-medium shadow-sm">
            ...
            <h1 className="text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-bold text-brown-900 leading-tight">
              {heroSettings.title.includes(' ') ? (
                <>
                  {heroSettings.title.split(' ').slice(0, -2).join(' ')}{' '}
                  <span className="text-primary">{heroSettings.title.split(' ').slice(-2).join(' ')}</span>
                </>
              ) : (
                heroSettings.title
              )}
            ...
            {/* Subtitle */}
            <p className="text-base sm:text-lg text-brown-600 max-w-xl mx-auto lg:mx-0 leading-relaxed">
            ...
            {heroSettings.showTrustBadges && heroSettings.trustBadges && (
              <div className="grid grid-cols-3 sm:flex sm:flex-wrap gap-3 sm:gap-6 justify-center lg:justify-start pt-2 sm:pt-4">
                {heroSettings.trustBadges.map((badge: { text: string; icon?: string }, index: number) => {
                  ...
                  return (
                    <div key={index} className="flex flex-col sm:flex-row items-center gap-1.5 sm:gap-2 text-xs sm:text-sm text-brown-600">
          ...
          <div className="relative order-1 lg:order-2">
            <div className="relative aspect-square max-w-[280px] sm:max-w-md lg:max-w-lg mx-auto">
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-primary/20 to-primary/5 rounded-2xl sm:rounded-3xl transform rotate-3" />
                ...
                {store?.storeLogo ? (
                  ...
                  <div className="w-full h-full bg-gradient-to-br from-brown-100 to-brown-50 flex items-center justify-center">
                    <div className="text-center p-6 sm:p-8">
                      <div className="w-20 h-20 sm:w-32 sm:h-32 mx-auto mb-3 sm:mb-4 bg-primary/10 rounded-full flex items-center justify-center">
                        <Gift className="h-10 w-10 sm:h-16 sm:w-16 text-primary" />
                      </div>
                      <p className="text-brown-600 font-medium text-sm sm:text-base">Premium Pet Shop</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="hidden sm:block absolute -top-4 -right-4 bg-white rounded-xl sm:rounded-2xl shadow-lg p-3 sm:p-4 animate-bounce">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xs sm:text-sm font-bold text-brown-900">4.9/5</p>
                    <p className="text-[10px] sm:text-xs text-brown-500">Rating</p>
                  </div>
                </div>
              </div>
              ...
              <div className="hidden sm:block absolute -bottom-4 -left-4 bg-white rounded-xl sm:rounded-2xl shadow-lg p-3 sm:p-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xs sm:text-sm font-bold text-brown-900">1000+</p>
                    <p className="text-[10px] sm:text-xs text-brown-500">Products</p>
                  </div>
                </div>
              </div>
            </div>
